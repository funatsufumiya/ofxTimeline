...
kiss_fftr_cfg ofOpenALSoundPlayer_Timeline::systemFftCfg=0;
vector<float> ofOpenALSoundPlayer_Timeline::systemWindowedSignal;
vector<float> ofOpenALSoundPlayer_Timeline::systemBins;
vector<kiss_fft_cpx> ofOpenALSoundPlayer_Timeline::systemCx_out;

static set<ofOpenALSoundPlayer_Timeline*> & players(){
    static set<ofOpenALSoundPlayer_Timeline*> * players = new set<ofOpenALSoundPlayer_Timeline*>;
	return *players;
}

//void ofOpenALSoundUpdate(){
//    alcProcessContext(ofOpenALSoundPlayer_Timeline::alContext);
//}
...
//------------------------------------------------------------
ofOpenALSoundPlayer_Timeline::ofOpenALSoundPlayer_Timeline(){
	bLoop 			= false;
	bLoadedOk 		= false;
...
//---------------------------------------
void ofOpenALSoundPlayer_Timeline::createWindow(int size){
...
//---------------------------------------
void ofOpenALSoundPlayer_Timeline::close(){
	if(alDevice){
...
bool ofOpenALSoundPlayer_Timeline::sfReadFile(const std::filesystem::path& path, vector<short> & buffer, vector<float> & fftAuxBuffer){
	...
	if (subformat == SF_FORMAT_FLOAT || subformat == SF_FORMAT_DOUBLE){
		...
		if(samples_read<(int)fftAuxBuffer.size()){
            ofLogWarning("ofOpenALSoundPlayer_Timeline") << "sfReadFile(): read " << samples_read << " float samples, expected "
		...
		if(frames_read<sfInfo.frames){
            ofLogError("ofOpenALSoundPlayer_Timeline") << "sfReadFile(): read " << frames_read << " frames from buffer, expected "
		...
		if(frames_read<sfInfo.frames){
            ofLogError("ofOpenALSoundPlayer_Timeline") << "sfReadFile(): read " << frames_read << " frames from fft buffer, expected "
			<< sfInfo.frames << " for \"" << path << "\"";
			return false;
		}
	}
...
#ifdef OF_USING_MPG123
//------------------------------------------------------------
bool ofOpenALSoundPlayer_Timeline::mpg123ReadFile(const std::filesystem::path& path,vector<short> & buffer,vector<float> & fftAuxBuffer){
	int err = MPG123_OK;
	mpg123_handle * f = mpg123_new(nullptr,&err);
	if(mpg123_open(f,path.c_str())!=MPG123_OK){
        ofLogError("ofOpenALSoundPlayer_Timeline") << "mpg123ReadFile(): couldn't read \"" << path << "\"";
		return false;
	}
	...
	if(encoding!=MPG123_ENC_SIGNED_16){
        ofLogError("ofOpenALSoundPlayer_Timeline") << "mpg123ReadFile(): " << getMpg123EncodingString(encoding)
...
#ifdef OF_USING_MPG123
//------------------------------------------------------------
bool ofOpenALSoundPlayer_Timeline::mpg123Stream(const std::filesystem::path& path,vector<short> & buffer,vector<float> & fftAuxBuffer){
	if(!mp3streamf){
		int err = MPG123_OK;
		...
		if(mpg123_open(mp3streamf,path.c_str())!=MPG123_OK){
            ...
            ofLogError("ofOpenALSoundPlayer_Timeline") << "mpg123Stream(): couldn't read \"" << path << "\"";
			return false;
		}
		...
		if(stream_encoding!=MPG123_ENC_SIGNED_16){
            ofLogError("ofOpenALSoundPlayer_Timeline") << "mpg123Stream(): " << getMpg123EncodingString(stream_encoding)
			<< " encoding for \"" << path << "\"" << " unsupported, expecting MPG123_ENC_SIGNED_16";
			return false;
		}
		samplerate = rate;
...
//------------------------------------------------------------
bool ofOpenALSoundPlayer_Timeline::stream(const std::filesystem::path& fileName, vector<short> & buffer){
#ifdef OF_USING_MPG123
...
bool ofOpenALSoundPlayer_Timeline::readFile(const std::filesystem::path& fileName, vector<short> & buffer){
#ifdef OF_USING_MPG123
...
//------------------------------------------------------------
bool ofOpenALSoundPlayer_Timeline::load(const std::filesystem::path& _fileName, bool is_stream){
	...
	if(channels==1){
		...
		err = alGetError();
		if (err != AL_NO_ERROR){
            ofLogError("ofOpenALSoundPlayer_Timeline") << "loadSound(): couldn't generate source for \"" << fileName << "\": "
		...
		for(int i=0; i<(int)buffers.size(); i++){
			...
			err = alGetError();
			if (err != AL_NO_ERROR){
                ofLogError("ofOpenALSoundPlayer_Timeline:") << "loadSound(): couldn't create buffer for \"" << fileName << "\": "
				<< (int) err << " " << getALErrorString(err);
				return false;
			}
			if(isStreaming){
		...
		if(isStreaming){
			for(int s=0; s<2;s++){
				for(int i=0;i<channels;i++){
					...
					err = alGetError();
					if ( err != AL_NO_ERROR){
                        ofLogError("ofOpenALSoundPlayer_Timeline") << "loadSound(): couldn't create stereo buffers for \"" << fileName << "\": " << (int) err << " " << getALErrorString(err);
						return false;
					}
			...
			for(int i=0;i<channels;i++){
				...
				err = alGetError();
				if (err != AL_NO_ERROR){
                    ofLogError("ofOpenALSoundPlayer_Timeline") << "loadSound(): couldn't create stereo buffers for \"" << fileName << "\": "
					<< (int) err << " " << getALErrorString(err);
					return false;
				}
		...
		for(int i=0;i<channels;i++){
			err = alGetError();
			if (err != AL_NO_ERROR){
                ofLogError("ofOpenALSoundPlayer_Timeline") << "loadSound(): couldn't create stereo sources for \"" << fileName << "\": "
				<< (int) err << " " << getALErrorString(err);
				return false;
			}
...
//------------------------------------------------------------
bool ofOpenALSoundPlayer_Timeline::isLoaded() const{
	return bLoadedOk;
}

//------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::threadedFunction(){
	vector<vector<short> > multibuffer;
	multibuffer.resize(channels);
...
//------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::update(ofEventArgs & args){
...
//------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::unload(){
	stop();
    ofRemoveListener(ofEvents().update,this,&ofOpenALSoundPlayer_Timeline::update);
...
//------------------------------------------------------------
bool ofOpenALSoundPlayer_Timeline::isPlaying() const{
...
//------------------------------------------------------------
bool ofOpenALSoundPlayer_Timeline::isPaused() const{
	if(sources.empty()) return false;
	ALint state;
	bool paused=true;
	for(int i=0;i<(int)sources.size();i++){
...
//------------------------------------------------------------
float ofOpenALSoundPlayer_Timeline::getSpeed() const{
	return speed;
}

//------------------------------------------------------------
float ofOpenALSoundPlayer_Timeline::getPan() const{
	return pan;
}
...
//------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::setPosition(float pct){
	setPositionMS(duration*pct*1000.f);
}

//------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::setPositionMS(int ms){
	if(sources.empty()) return;
#ifdef OF_USING_MPG123
	if(mp3streamf){
...
//------------------------------------------------------------
float ofOpenALSoundPlayer_Timeline::getPosition() const{
...
//------------------------------------------------------------
int ofOpenALSoundPlayer_Timeline::getPositionMS() const{
...
//------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::setPan(float p){
	if(sources.empty()) return;
...
//------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::setPaused(bool bP){
	if(sources.empty()) return;
	std::unique_lock<std::mutex> lock(mutex);
	if(bP){
...
//------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::setSpeed(float spd){
	for(int i=0;i<channels;i++){
...
//------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::setLoop(bool bLp){
...
// ----------------------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::setMultiPlay(bool bMp){
	if(isStreaming && bMp){
        ofLogWarning("ofOpenALSoundPlayer_Timeline") << "setMultiPlay(): sorry, no support for multiplay streams";
		return;
	}
	bMultiPlay = bMp;		// be careful with this...
	if(sources.empty()) return;
	if(bMultiPlay){
        ofAddListener(ofEvents().update,this,&ofOpenALSoundPlayer_Timeline::update);
	}else{
        ofRemoveListener(ofEvents().update,this,&ofOpenALSoundPlayer_Timeline::update);
	}
}
...
// ----------------------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::play(){
	std::unique_lock<std::mutex> lock(mutex);
	int err = glGetError();
	...
	if (bMultiPlay && !bLoop){
		...
		err = alGetError();
		if (err != AL_NO_ERROR){
            ofLogError("ofOpenALSoundPlayer_Timeline") << "play(): couldn't create multiplay stereo sources: "
			<< (int) err << " " << getALErrorString(err);
			return;
		}
		for(int i=0;i<channels;i++){
		...
		err = glGetError();
		if (err != AL_NO_ERROR){
            ofLogError("ofOpenALSoundPlayer_Timeline") << "play(): couldn't assign multiplay buffers: "
			<< (int) err << " " << getALErrorString(err);
			return;
		}
	}
...
// ----------------------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::stop(){
	if(sources.empty()) return;
	std::unique_lock<std::mutex> lock(mutex);
...
// ----------------------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::initFFT(int bands){
...
// ----------------------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::initSystemFFT(int bands){
...
float * ofOpenALSoundPlayer_Timeline::getCurrentBufferSum(int size){
...
// ----------------------------------------------------------------------------
float * ofOpenALSoundPlayer_Timeline::getSpectrum(int bands){
	initFFT(bands);
...
// ----------------------------------------------------------------------------
float * ofOpenALSoundPlayer_Timeline::getSystemSpectrum(int bands){
	initSystemFFT(bands);
    ...
    set<ofOpenALSoundPlayer_Timeline*>::iterator it;
...
// ----------------------------------------------------------------------------
void ofOpenALSoundPlayer_Timeline::runWindow(vector<float> & signal){
